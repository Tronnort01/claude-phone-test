...
- **2026-04-20 Agent compatibility (`a61361c..affb23c`):** Audited `agent/` and `server/` for Round 8 command gaps. Server needs no changes (generic relay accepts any `type: String`). Agent had zero command handling — added `AgentCameraService` (Camera2 still capture via `CountDownLatch`, upload as `ByteArray`), `AgentLiveCameraService` (live JPEG WebSocket stream to `/camera/{side}/{deviceId}`), `AgentScreenRecordService` (MediaCodec H.264 + MediaMuxer, `FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION` added to manifest), `AgentNotificationListener` inline reply via `RemoteInput.addResultsToIntent`, plus `record_audio`, `send_sms`, `launch_app`, `ring`, `lock_device`, `wipe_vault` inline in `AgentForegroundService`. All 13 commands now dispatched. See `docs/ISSUES_FOUND.md` and `docs/FIX_PLAN.md` "Agent + Server Compatibility" sections.
- **2026-04-20 Round 8 (`b19bcb0`, branch `claude/round8-features-bYw2Y`):** 16 new features. AMOLED true-black theme toggle; app icon switcher (3 `<activity-alias>` entries: Calculator/Clock/Notes); biometric-only unlock via long-press `=` (BiometricHelper migrated to ComponentActivity); shake sensitivity picker (15/25/35 m/s²); clipboard timeout config (15s–Never); thumbnail regeneration button in vault; OCR on photos via ML Kit TextRecognition (new dep `text-recognition:16.0.1`) with copy dialog; remote lock (`lock_device` → `DevicePolicyManager.lockNow()`); scheduled silent windows (AgentService reads schedule prefs directly — ViewModel can't inject into Service); GeckoView UA override; decoy PIN vault wipe; browser save-page-to-vault (URL stub); remote wipe (`wipe_vault` → `WipeManager.wipeAll()`); timeline view (`TimelineScreen` + `TimelineViewModel`, events grouped by hour). See `docs/ROUND8_FEATURES.md` for full detail.
